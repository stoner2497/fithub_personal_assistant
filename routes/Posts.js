...
router.get(
  ...
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    BLOG.findOne({ User: req.user.id }).then(blogs => {
      if (blogs.length == []) {
...
router.post(
  ...
  multer.single("post"),
  async (req, res) => {
    Account.findOne({ user: req.user.id }).then(async account => {
      if (!account) {
    ...
    Account.findOne({ user: req.user.id })
      .then(account => {
        ...
        if (isEmpty(account)) {
          ...
          const postsPromises = Subscribing.map(user =>
            Post.find({ User: user.user })
          );
          Promise.all(postsPromises).then((...data) => {
            if (!data) {
...
router.get(
  ...
  async (req, res) => {
    Account.findOne({ user: req.user.id })
      .then(account => {
        if (isEmpty(account)) {
          res.json({ msg: "please create account" });
        } else {
          let results;
          const { Subscribing } = account;
          const blogsPromises = Subscribing.map(user =>
            BLOG.find({ User: user.user })
          );
          Promise.all(blogsPromises).then((...data) => {
            if (isEmpty(...data)) {
              console.log(...data);
              res.json({ msg: "no posts from your subscriber" });
            } else {
